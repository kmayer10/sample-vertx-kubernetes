node('ravi-paajee') {
	...
	stage 'Scan Image using Trivy'
		sh 'rpm -ivh https://github.com/aquasecurity/trivy/releases/download/v0.9.2/trivy_0.9.2_Linux-64bit.rpm && trivy image --exit-code 1 --severity HIGH,CRITICAL docker.io/kulbhushanmayer/node-app:v1.0'
